...
def _etapas_para_reprocessar(
    ...
    # Seleção robusta por versão (ID) do documento: reprocessa apenas
    # o que mudou desde o último processamento concluído.
    log_atual = validacao.log_ia if isinstance(validacao.log_ia, dict) else {}
    etapas_log = log_atual.get("etapas") if isinstance(log_atual.get("etapas"), dict) else {}
    orq_log = etapas_log.get("orquestracao") if isinstance(etapas_log.get("orquestracao"), dict) else {}
    ids_processados = (
        orq_log.get("doc_ids_processados")
        if isinstance(orq_log.get("doc_ids_processados"), dict)
        else {}
    )
    arquivos_processados = (
        orq_log.get("doc_arquivos_processados")
        if isinstance(orq_log.get("doc_arquivos_processados"), dict)
        else {}
    )
    ids_atuais = {
        "LAUDO": doc_laudo.id if doc_laudo else None,
        "IDENTIDADE": doc_tea.id if doc_tea else None,
        "ENDERECO": doc_end.id if doc_end else None,
        "RESPONSAVEL": doc_resp.id if doc_resp else None,
    }
    arquivos_atuais = {
        "LAUDO": doc_laudo.arquivo.name if doc_laudo and doc_laudo.arquivo else None,
        "IDENTIDADE": doc_tea.arquivo.name if doc_tea and doc_tea.arquivo else None,
        "ENDERECO": doc_end.arquivo.name if doc_end and doc_end.arquivo else None,
        "RESPONSAVEL": doc_resp.arquivo.name if doc_resp and doc_resp.arquivo else None,
    }

    primeira_execucao = not bool(ids_processados)
    ...
    etapas = []
    if ids_atuais["LAUDO"] and (
        ids_atuais["LAUDO"] != ids_processados.get("LAUDO")
        or arquivos_atuais["LAUDO"] != arquivos_processados.get("LAUDO")
    ):
        etapas.append("LAUDO")
    if ids_atuais["IDENTIDADE"] and (
        ids_atuais["IDENTIDADE"] != ids_processados.get("IDENTIDADE")
        or arquivos_atuais["IDENTIDADE"] != arquivos_processados.get("IDENTIDADE")
    ):
        etapas.append("IDENTIDADE")
    if ids_atuais["ENDERECO"] and (
        ids_atuais["ENDERECO"] != ids_processados.get("ENDERECO")
        or arquivos_atuais["ENDERECO"] != arquivos_processados.get("ENDERECO")
    ):
        etapas.append("ENDERECO")
    if ids_atuais["RESPONSAVEL"] and (
        ids_atuais["RESPONSAVEL"] != ids_processados.get("RESPONSAVEL")
        or arquivos_atuais["RESPONSAVEL"] != arquivos_processados.get("RESPONSAVEL")
    ):
        etapas.append("RESPONSAVEL")

    # Sem mudanças detectadas: evita reprocessar o lote inteiro.
    # O fluxo seguirá apenas para finalize, preservando resultados existentes.
    if not etapas:
        etapas = ["LAUDO", "IDENTIDADE", "ENDERECO"] if primeira_execucao else []
        if primeira_execucao and doc_resp:
            etapas.append("RESPONSAVEL")
    return etapas
...
def _usar_classificador_completo(ocr: dict, result: dict) -> bool:
    # Modo clássico: sempre classifica.
    if not IA_FAST_PATH:
        return True
...
def process_ciptea_document_step(self, solicitacao_id: int, etapa: str):
    ...
    try:
        ...
        if _usar_classificador_completo(ocr, result):
            ...
            feat = extract_document_features(
                cfg["expected"], cls.get("predicted_type", ""), ocr.get("text", "")
            )
            cls["strategy"] = "full_classifier"
        else:
            # Fast-path: usa tipo esperado e só consulta classificador completo quando necessário.
            cls = {
                "predicted_type": cfg["expected"],
                "confidence": 1.0,
                "should_block": False,
                "strategy": "fast_path_expected_type",
            }
            feat = extract_document_features(cfg["expected"], cfg["expected"], ocr.get("text", ""))
...
def finalize_ciptea_triagem(self, solicitacao_id: int):
    ...
    doc_ids_alvo = orq.get("doc_ids_alvo") if isinstance(orq.get("doc_ids_alvo"), dict) else {}
    if doc_ids_alvo:
        orq["doc_ids_processados"] = doc_ids_alvo
    doc_arquivos_alvo = orq.get("doc_arquivos_alvo") if isinstance(orq.get("doc_arquivos_alvo"), dict) else {}
    if doc_arquivos_alvo:
        orq["doc_arquivos_processados"] = doc_arquivos_alvo
